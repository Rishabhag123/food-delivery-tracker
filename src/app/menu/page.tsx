...
function AddMenuItemCard({ onAdded }: { onAdded: () => void }) {
  const [form, setForm] = useState({ date: '', title: '', price: '', description: '' });
  async function handleAdd(e: React.FormEvent) {
    e.preventDefault();
    const { date, title, price, description } = form;
    if (!date || !title || !price) return alert('Date, title, and price are required');
    const { error } = await supabase.from('menu_items').insert([{ date, title, price: parseFloat(price), description }]);
    if (error) return alert('Error adding menu item');
    setForm({ date: '', title: '', price: '', description: '' });
    onAdded();
  }
  return (
...
export default function MenuPage() {
  ...
  const [editForm, setEditForm] = useState({ date: '', title: '', price: '', description: '' });
  ...
  function startEdit(item: any) {
    ...
    setEditForm({ date: item.date, title: item.title, price: item.price.toString(), description: item.description || '' });
  }

  async function handleEditSave(id: string) {
    const { date, title, price, description } = editForm;
    if (!date || !title || !price) return alert('Date, title, and price are required');
    const { error } = await supabase.from('menu_items').update({ date, title, price: parseFloat(price), description }).eq('id', id);
              ...
              <table className="min-w-full bg-white rounded-lg shadow divide-y divide-gray-200">
                <thead className="bg-white">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-black uppercase">Description</th>
